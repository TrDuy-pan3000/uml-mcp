...
async function getOrCreateModelId() {
    ...
    throw new Error("Could not determine project root model ID. " +
        "Make sure a StarUML project is open and the extension is running.");
}
async function ensureDiagram(name) {
    // Check if diagram already exists
    try {
        const diagrams = await api.getAllDiagramsInfo();
        if (Array.isArray(diagrams)) {
            const existing = diagrams.find((d) => d.name === name);
            if (existing)
                return existing;
...
async function addAttribute(modelId, attrText) {
...
async function addOperation(modelId, methodText) {
...
export function registerDiagramTools(server) {
    server.tool("generate_diagram", "Generate a diagram in StarUML from Mermaid syntax. Supports classDiagram. Elements are automatically positioned to avoid overlap.", {
        ...
        try {
            ...
            if (mermaid.nodes.length === 0) {
                return response.error(JsonRpcErrorCode.InvalidParams, "No elements found in Mermaid code.");
            }
            ...
            if (await api.isExtAvailable()) {
                try {
                    ...
                    const diag = Array.isArray(diagrams)
                        ? diagrams.find((d) => d.name === "Class Diagram by Mermaid") || diagrams[diagrams.length - 1]
                        : diagrams;
                            ...
                            try {
                                ...
                                if (model) {
                                    for (const attr of node.attributes)
                                        await addAttribute(model._id, attr);
                                    for (const method of node.methods)
                                        await addOperation(model._id, method);
                                }
                            }
                            catch { /* best-effort */ }
                        }
            ...
            if (postProcessed) {
                return response.text(`Diagram generated with ${mermaid.nodes.length} elements and ${mermaid.edges.length} connections. Dagre layout + rectilinear edges applied.`);
            }
            ...
            if (args.layout) {
                try {
                    ...
                    return response.text(`Diagram generated with ${mermaid.nodes.length} elements. Auto-layout applied.`);
                }
                catch {
                    // auto-layout failed, still return success
                }
            }
            return response.text(`Diagram generated with ${mermaid.nodes.length} elements.`);
        }
        catch (error) {
    ...
    server.tool("execute_command", "Execute any built-in StarUML command. Requires staruml-mcp-extension.", {
        ...
        try {
            ...
            return response.error(JsonRpcErrorCode.InternalError, `Failed to execute command: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    server.tool("list_commands", "List all available built-in StarUML commands. Requires staruml-mcp-extension.", {}, async ({}) => {
        try {
            const commands = await api.getAllCommands();
            return response.text(`Available commands: ${JSON.stringify(commands)}`);
        }
        catch (error) {
            return response.error(JsonRpcErrorCode.InternalError, `Failed to list commands: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
}
