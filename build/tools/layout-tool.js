...
export function registerLayoutTools(server) {
    server.tool("apply_layout", "Apply auto-layout to the current diagram. Supports direction and spacing options.", {
        ...
        try {
            ...
            try {
                ...
                if (res.ok) {
                    return response.text(`Auto-layout applied (direction: ${args.direction || "TB"}).`);
                }
            }
            catch {
                // fall through
            ...
            try {
                ...
                return response.text("Auto-layout could not be applied. Use Format > Auto Layout in StarUML, or install the mcp-layout-bridge extension.");
            }
        }
        catch (error) {
            return response.error(JsonRpcErrorCode.InternalError, `Layout failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    server.tool("arrange_elements", "Arrange elements on the current diagram using a dagre layout algorithm for optimal positioning.", {
        ...
        try {
            ...
            return response.text(`Arranged ${moved} of ${nodes.length} elements using ${args.direction || "TB"} layout.`);
        }
        catch (error) {
            return response.error(JsonRpcErrorCode.InternalError, `Arrange failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
}
