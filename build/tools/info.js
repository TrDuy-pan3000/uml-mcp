...
export function registerInfoTools(server) {
    ...
    server.tool("get_diagram_image_by_id", "Get the image of a diagram by its ID in StarUML.", {
        ...
        try {
            ...
            return response.error(JsonRpcErrorCode.InternalError, `Failed to get diagram image: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    server.tool("get_project_info", "Get information about the current StarUML project.", {}, async ({}) => {
        try {
            const data = await api.getProjectInfo();
            return response.text(`Project info: ${JSON.stringify(data)}`);
        }
        catch (error) {
            return response.error(JsonRpcErrorCode.InternalError, `Failed to get project info: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    server.tool("get_diagram_ids", "List all diagram names and IDs in the StarUML project.", {}, async ({}) => {
        try {
            const data = await api.getAllDiagramsInfo();
            const list = Array.isArray(data)
                ? data.map((d) => ({ id: d.id || d._id, name: d.name }))
                : [{ id: data.id || data._id, name: data.name }];
            return response.text(`Diagrams:\n${list.map((d) => `  - ${d.name} (id: ${d.id})`).join("\n")}`);
        }
        catch (error) {
            return response.error(JsonRpcErrorCode.InternalError, `Failed to list diagrams: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    server.tool("get_diagram_summary", "Get a complete summary of a diagram: all nodes with positions, attributes, methods, types, and all edges with connections. Requires staruml-mcp-extension.", {
        ...
        try {
            ...
            if (nodes.length > 0) {
                ...
                for (const n of nodes) {
                    ...
                    const attrs = elem?.ownedElements?.filter((o) => o.name)?.map((o) => o.name) ?? [];
            ...
            return response.error(JsonRpcErrorCode.InternalError, `Failed to get diagram summary: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
}
