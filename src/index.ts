...
type LayoutDirection = "TB" | "LR" | "BT" | "RL" | "custom";
...
server.tool(
  "generate_diagram",
  "Generate a diagram in StarUML from Mermaid syntax. Optionally auto-arranges elements after generation.",
  {
    code: z
      .string()
      .describe(
    ...
    layout: z
      ...
      .default(true)
      .describe(
        "Whether to automatically apply auto-layout after diagram generation to neatly arrange all elements. Default is true."
      ),
    layoutDirection: z
      ...
      .default("TB")
      .describe(
        "Layout direction for auto-layout: TB (top-to-bottom), LR (left-to-right), BT (bottom-to-top), RL (right-to-left). Default is TB."
      ),
    nodeSpacing: z
      .number()
      .int()
  ...
  async ({ code, layout, layoutDirection, nodeSpacing, rankSpacing }) => {
    try {
      ...
      if (layout) {
        const layoutMessage = await applyLayout(
...
server.tool(
  "layout_diagram",
  "Apply auto-layout to the current diagram in StarUML to neatly arrange all elements. Use this after generating or modifying a diagram.",
  {
    direction: z
      .enum(["TB", "LR", "BT", "RL", "custom"])
      .optional()
      .default("TB")
      .describe(
        "Layout direction: TB (top-to-bottom), LR (left-to-right), BT (bottom-to-top), RL (right-to-left). Default is TB."
      ),
    nodeSpacing: z
      .number()
      .int()
