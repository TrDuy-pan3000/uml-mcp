...
const UML_TO_MERMAID_EDGE: Record<string, string> = {
  UMLAssociation: "-->",
  UMLGeneralization: "<|--",
...
function buildMermaidFromSummary(summary: any): string {
  ...
  for (const n of nodes) {
    ...
    const owned = elem?.ownedElements?.filter((o: any) => o.name) ?? [];
    const attrs = owned.filter((o: any) => o._type !== "UMLOperation" || !o._type);
    const ops = owned.filter((o: any) => o._type === "UMLOperation");
...
function mergeNodes(existing: any[], newNodes: any[]): any[] {
...
function mergeEdges(existing: any[], newEdges: any[]): any[] {
...
function mermaidFromMerged(nodes: any[], edges: any[]): string {
  const lines: string[] = ["classDiagram"];
  for (const n of nodes) {
    if (n.type === "interface") {
    ...
    if (n.attributes.length > 0 || n.methods.length > 0) {
      lines.push("{");
      for (const a of n.attributes) lines.push(`  ${a}`);
...
export function registerAddToDiagramTool(server: any) {
  server.tool(
    "add_to_diagram",
    "Add new classes and relationships from Mermaid code into an existing diagram. Reads current diagram, merges with new elements, regenerates with proper layout. Requires staruml-mcp-extension.",
    {
      ...
      try {
        if (!(await api.isExtAvailable())) {
          return response.error(
            ...
            "staruml-mcp-extension is required."
          );
        }
        ...
        if (newParsed.nodes.length === 0) {
          return response.error(
            ...
            "No new elements found in Mermaid code."
          );
        }

        // Step 3: Build existing nodes/edges from summary
        const existingNodes: any[] = [];
        for (const v of summary.views.filter((v: any) => v.kind === "node")) {
          const elem = summary.elements.find((e: any) => e._id === v.modelId);
          const owned = elem?.ownedElements?.filter((o: any) => o.name) ?? [];
          const attrs = owned.filter((o: any) => o._type !== "UMLOperation" || !o._type).map((o: any) => o.name);
          const ops = owned.filter((o: any) => o._type === "UMLOperation").map((o: any) => o.name + "()");
          existingNodes.push({
            id: v.name,
            label: v.name,
        ...
        for (const v of summary.views.filter((v: any) => v.kind === "edge")) {
          existingEdges.push({
            sourceId: v.tailName,
            targetId: v.headName,
        ...
        const existingNames = new Set(existingNodes.map((n: any) => n.label));
        const trulyNewNodes = newParsed.nodes.filter((n: any) => !existingNames.has(n.label));
        const trulyNewEdges = newParsed.edges.filter((e: any) => {
          const key = `${e.sourceId}|${e.targetId}|${e.type}`;
          return !existingEdges.some((ee: any) => `${ee.sourceId}|${ee.targetId}|${ee.type}` === key);
        });
        ...
        const layout = computeLayout(mergedDiagram, {
          direction: (args.layoutDirection as any) || DEFAULT_LAYOUT_DIRECTION,
        ...
        try {
          ...
          for (const node of mergedNodes) {
            ...
            if (!pos) continue;
            const view = views.find((v: any) => v.kind === "node" && v.name === node.label);
            if (view) {
          ...
          return response.text(
            `Diagram "${oldDiagramName}" extended: added ${trulyNewNodes.length} elements and ${trulyNewEdges.length} connections. New diagram id: ${newDiagramId}`
          );
        } catch {
          if (postProcessed) {
            return response.text(
              `Diagram extended but old diagram may not have been deleted.`
            );
          }
          // Fallback: return success without post-processing
          return response.text(
            `Diagram regenerated with ${mergedNodes.length} elements and ${mergedEdges.length} connections (post-processing skipped).`
          );
        }
      } catch (error) {
        return response.error(
          JsonRpcErrorCode.InternalError,
          `Failed to extend diagram: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
...
function edgeTypeFromUml(umlType: string): string {
