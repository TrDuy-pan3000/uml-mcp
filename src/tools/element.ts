...
export function registerElementTools(server: any) {
  server.tool(
    "create_element",
    "Create a UML model element WITH its visual view on a diagram. Supports x/y positioning. Requires staruml-mcp-extension.",
    {
      ...
      try {
        if (!(await api.isExtAvailable())) {
          return response.error(
            ...
            "staruml-mcp-extension is required. Install it via StarUML: Tools > Extension Manager > Install From URL > https://github.com/ezrabrilliant/staruml-mcp-extension"
          );
        }
  ...
  server.tool(
    "create_edge",
    "Connect two existing views on a diagram with a typed relationship edge. Requires staruml-mcp-extension.",
    {
  ...
  server.tool(
    "update_element",
    "Set a property on an existing element. Requires staruml-mcp-extension.",
    {
    ...
    async (args: { id: string; field: string; value: unknown }) => {
      try {
  ...
  server.tool(
    "delete_element",
    "Delete an element by its ID. Requires staruml-mcp-extension.",
    {
  ...
  server.tool(
    "get_element_by_id",
    "Get details of a specific element by its ID. Requires staruml-mcp-extension.",
    {
      id: z.string().describe("Element _id"),
    },
    async (args: { id: string }) => {
      try {
        ...
        return response.error(
          JsonRpcErrorCode.InternalError,
          `Failed to get element: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
  ...
  server.tool(
    "find_elements",
    "Search for elements by name or type. Requires staruml-mcp-extension.",
    {
  ...
  server.tool(
    "update_class",
    "Update a UML class: rename, set abstract, add/remove attributes and methods, move view. Requires staruml-mcp-extension.",
    {
    ...
    async (args: {
      ...
      try {
        ...
        if (args.removeAttributes?.length) {
          ...
          const owned = (detail?.ownedElements ?? []) as any[];
          for (const attrName of args.removeAttributes) {
            const target = owned.find((o: any) => o.name === attrName && !o.name?.endsWith("View"));
            if (target?._id) {
              try {
        ...
        if (args.removeMethods?.length) {
          ...
          for (const methodName of args.removeMethods) {
            const target = owned.find((o: any) => o.name === methodName && !o.name?.endsWith("View"));
            if (target?._id) {
              try {
                await api.deleteElement(target._id);
                changes.push(`-method "${methodName}"`);
        ...
        if (args.viewId && (args.left !== undefined || args.top !== undefined)) {
          const moveArgs: any = { id: args.viewId };
          if (args.left !== undefined) moveArgs.left = args.left;
          if (args.top !== undefined) moveArgs.top = args.top;
        ...
        return response.error(
          JsonRpcErrorCode.InternalError,
          `Failed to update class: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  );
