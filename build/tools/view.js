...
export function registerViewTools(server) {
    server.tool("get_diagram_views", "List all visual views on a diagram with their positions (left, top, width, height). Requires staruml-mcp-extension.", {
        ...
        try {
            ...
            return response.error(JsonRpcErrorCode.InternalError, `Failed to get views: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    server.tool("move_view", "Move a visual view to a specific position on the diagram. Requires staruml-mcp-extension.", {
        ...
        try {
            ...
            return response.error(JsonRpcErrorCode.InternalError, `Failed to move view: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    server.tool("resize_view", "Resize a visual view on the diagram. Requires staruml-mcp-extension.", {
        viewId: z.string().describe("View _id"),
        ...
        try {
            ...
            return response.error(JsonRpcErrorCode.InternalError, `Failed to resize view: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    server.tool("align_views", "Align multiple views relative to each other. Requires staruml-mcp-extension.", {
        ...
        try {
            ...
            return response.error(JsonRpcErrorCode.InternalError, `Failed to align views: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
}
